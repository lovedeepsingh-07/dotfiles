...
function M.delete_file(fs_t, ask)
	...
	local prompt = vim.fn.input(string.format("Confirm deletion of (%s) {yes,n(o),q(uit)}: ", fs_t.filename), "")
	if prompt == "yes" then
		if fs_t.filetype == "directory" then
			fs.do_delete(fs_t.filepath)
		else
...
function M.duplicate_file(fs_t)
	...
	if fs.file_exists(destination_path) then
		vim.notify(string.format(' DiredDuplicate: File "%s" already exists.', new_name), "error")
		return
	end
	...
	if not success then
		vim.notify(
			string.format(
				' DiredDuplicate: Could not duplicate "%s" to "%s". %s',
				fs_t.filename,
				new_name,
				errmsg or ""
			),
	...
	display.goto_filename = new_name
	vim.notify(string.format(' DiredDuplicate: "%s" duplicated as "%s"', fs_t.filename, new_name))
end
